...
setwd("D:\\Documents\\Study and small Project training\\project_tsunami\\test\\bmkg")
options(scipen = 999)
...
data <- read.csv("Gempa_Tsunami_BMKG_Bathy.csv")
...
ggplot(gather(data[5:ncol(data)]), aes(value)) + 
  geom_histogram(bins = 5, fill = "blue", alpha = 0.6) + 
  facet_wrap(~key, scales = 'free_x')

str(data)

train.aba <- cbind(data[, 5:ncol(data)])
...
nn <- neuralnet(f,
                ...
                act.fct = "logistic",
                linear.output = FALSE,
                lifesign = "minimal", threshold = 0.1,stepmax=1e7)
#stepmax=1e7
summary(nn)
...
# Compute predictions
predicted.nn <- neuralnet::compute(nn, train.aba[,1:12])
...
original_values <- max.col(train.aba[, 13])
...
for(i in 1:k){
  ...
  cross.nn    <- neuralnet(f,
                           ...
                           hidden = c(8,4,2),
                           act.fct = "logistic",
                           linear.output = FALSE, threshold = 0.08,stepmax=1e7)
  
  # Compute predictions
  predicted.nn <- neuralnet::compute(cross.nn, cross.test[, 1:12])
  
  # Extract results
  result.predicted.nn <- predicted.nn$net.result
  
  # Accuracy (test set)
  original_values <- max.col(cross.test[, 13])
  result.predicted.nn_2 <- max.col(result.predicted.nn)
...
results <- data.frame(actual = cross.test[13], prediction = predicted.nn$net.result)
